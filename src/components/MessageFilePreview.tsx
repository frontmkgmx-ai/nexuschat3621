...
const CustomAudioPlayer = ({ 
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      // Pausar outros elementos de áudio
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch(e => {
            ...
            console.warn("Attached audio play prevented:", e?.message || e?.name);
            setHasError(true);
            setErrorMessage('Tentar novamente');
          });
      }
    }
  };
    ...
    <div className="w-[280px] sm:w-[340px] max-w-full bg-[#1a1a1c] rounded-[20px] p-4 shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5 relative overflow-hidden group">
      ...
      <div className="flex items-center gap-4 relative z-10">
        <button 
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all transform active:scale-95 ${
            hasError ? 'bg-red-500/20 text-red-400' : 'bg-indigo-500 hover:bg-indigo-600 text-white'
          }`}
        >
          {hasError ? (
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="flex justify-between items-end mb-1">
            ...
            <span className="text-[10px] text-zinc-500 font-medium shrink-0">
              {hasError ? <span className="text-red-400">{errorMessage}</span> : sizeText}
            </span>
          </div>
          
          <div 
            className={`h-1.5 w-full bg-black/40 rounded-full overflow-hidden mt-2 relative ${hasError ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
            onClick={(e) => {
              if (audioRef.current && !hasError) {
                ...
                const dur = audioRef.current.duration;
                if (dur > 0) {
                  const clickedValue = Math.max(0, Math.min(dur, (x / rect.width) * dur));
                  audioRef.current.currentTime = clickedValue;
                  setProgress((clickedValue / dur) * 100);
                  setCurrentTime(formatTime(clickedValue));
                }
              }
            }}
          >
            <div 
              className={`h-full rounded-full transition-all duration-100 ease-linear ${hasError ? 'bg-red-400' : 'bg-indigo-500'}`} 
