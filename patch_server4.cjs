          ...
          try {
             ...
             for (const p of participants) {
                 if (p.name === "meu-agente") {
                     hasMyAgent = true;
                 } else if (p.identity.includes("agent") || p.identity.includes("voice_assistant")) {
                     // Kick any other agent that might have auto-joined (like LiveKit Sandbox agent)
...
console.log("server.ts patched to kick other agents");
